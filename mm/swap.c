...
{
	page = compound_head(page);
	if (PageAnon(page) && (!PageUnevictable(page)))
		write_lru_page(page_app_id(page), page_to_pfn(page), 0);
	if (lru_gen_enabled()) {
		page_inc_refs(page);
		return;
	}
...
void release_pages(struct page **pages, int nr)
{
	...
	for (i = 0; i < nr; i++) {
		...
		unsigned long app_id = page_app_id(page);
		...
		if (PageLRU(page)) {
			...
			del_page_from_lru_list(page, lruvec);
			write_lru_page(app_id, page_to_pfn(page), 1);
			__clear_page_lru_flags(page);
		}
				 ...
				 void *arg)
{
	...
	if ((page_lru(page) == LRU_ACTIVE_ANON) || (page_lru(page) == LRU_INACTIVE_ANON)) {
		write_lru_page(page_app_id(page), page_to_pfn(page), 0);
	}
	add_page_to_lru_list(page, lruvec);
